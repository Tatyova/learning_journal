...
SELECT courses.name, teachers.first_name, teachers.last_name
from teachers
JOIN courses on teachers.id=courses.teachers_id
order by courses.name ASC;
-- Display how many courses per universities
SELECT teachers.school, COUNT(courses.id) as Total_Courses
from teachers
JOIN courses on teachers.id=courses.teachers_id
group by teachers.school;
-- Display how many total_students per teachers
SELECT teachers.id, teachers.first_name, teachers.last_name, sum(courses.total_students) AS TotalStudents
from teachers
JOIN courses on teachers.id=courses.teachers_id
group by teachers.id;
-- Display how many courses per teachers
SELECT teachers.first_name, teachers.last_name, count(courses.id) AS Total_Matkul
from teachers
JOIN courses on teachers.id=courses.teachers_id
